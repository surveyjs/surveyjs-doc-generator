...
exports.addClassIntoJSONDefinition = void 0;
function addClassIntoJSONDefinition(ctx, className, isRoot) {
    if (isRoot === void 0) { isRoot = false; }
    if (className == "IElement") {
        className = "SurveyElement";
    }
    if (!!ctx.generateJSONDefinitionClasses[className])
        return;
    ...
    if (!cur || (!isRoot && hasClassInJSONDefinition(ctx, className)))
        return;
...
exports.addClassIntoJSONDefinition = addClassIntoJSONDefinition;
...
function hasClassInJSONDefinition(ctx, className) {
    return (!!ctx.outputDefinition["definitions"] &&
...
function getTypeValue(ctx, property) {
    ...
    if (childrenTypes.length == 1)
        return getReferenceType(ctx, childrenTypes[0]);
    if (childrenTypes.length > 1) {
        var res = [];
